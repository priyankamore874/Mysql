...
select weight from cats;
select round(weight) from cats;  -- row lavel functions
...
-- how many breeds are there in tables?
select count(distinct breed) from cats;
-- avg weight of every breed
...
select breed, round(avg(weight),2) from cats group by breed; -- output with 2 decimal values
-- find dep_id, average salary in each dept from my emp table
select * from myemp limit 10;
select dep_id ,avg(salary) from myemp group by dep_id;
-- find in each dept, under each manager whatis the avg salay
...
select * from movies;
select * from members;
select * from movies inner join members on movieid = id; -- for show common record use ON keyword 
select * from movies join members on movieid = id; -- writting inner keyword is optional
